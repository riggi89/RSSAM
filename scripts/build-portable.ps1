...
foreach ($targetArchitecture in $targetArchitectures) {
    ...
    $publishedAssembly = Join-Path $publishDirectory 'RSSAM.dll'
    $publishedVersionText = (Get-Item -LiteralPath $publishedAssembly).VersionInfo.FileVersion

    if ([string]::IsNullOrWhiteSpace($publishedVersionText)) {
        throw "RSSAM.dll in $runtimeIdentifier has no file version: $publishedAssembly"
    }

    $publishedVersion = [Version]$publishedVersionText
    if (
        $publishedVersion.Major -ne $expectedFileVersion.Major -or
        $publishedVersion.Minor -ne $expectedFileVersion.Minor -or
        $publishedVersion.Build -ne $expectedFileVersion.Build
    ) {
        throw (
            "Stale $runtimeIdentifier publish output. Expected RSSAM $version " +
            "but RSSAM.dll is version $publishedVersionText. Run without -SkipPublish."
        )
    }
